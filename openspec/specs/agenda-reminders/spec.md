...
The system SHALL send Brevo e-mail reminders for unwatched agenda items scheduled two days from the scheduled function run date.

#### Scenario: Matching item is found
...
#### Scenario: Item is not two days away
- **WHEN** an agenda item is scheduled for any date other than exactly two days after the scheduled function run date
...
The system SHALL prevent duplicate reminder e-mails for the same installation, agenda item, watch date, and reminder offset.

#### Scenario: Reminder was already sent
- **WHEN** a sent marker exists for the installation, agenda item, watch date, and two-day offset
- **THEN** the scheduled function MUST NOT send another e-mail for that reminder

#### Scenario: Reminder is sent successfully
- **WHEN** Brevo accepts a reminder e-mail request
- **THEN** the system MUST persist a sent marker for that reminder
